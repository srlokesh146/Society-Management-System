...
const DashboardTable = () => {
  ...
  const onDelete = () => {
    console.log("Deleted complaint:", currentComplaint);
  };
  ...
  return (
    <div className="bg-white pt-[20px] rounded-[15px] col-span-1 lg:col-span-2">
        ...
        <table className="w-full table-auto border-collapse">
          <thead>
            <tr className="text-start text-black bg-opacity-custom rounded-tl-[15px] rounded-tr-[15px] bg-gray-100 h-[61px] #F4F4F4">
              <th className="text-[14px] leading-[21px] font-semibold rounded-tl-[15px] text-start ps-[20px] max-sm:min-w-[180px] md:min-w-[180px] max-md:min-w-[180px]">Complainer Name</th>
              <th className="text-[14px] leading-[21px] font-semibold text-start max-sm:min-w-[180px] md:min-w-[166px] max-md:min-w-[180px]">Complaint Name</th>
              <th className="text-[14px] leading-[21px] font-semibold text-center max-sm:min-w-[180px] md:min-w-[166px] max-md:min-w-[180px]">Date</th>
              <th className="text-[14px] leading-[21px] font-semibold text-center max-sm:min-w-[180px] md:min-w-[166px] max-md:min-w-[180px]">Priority</th>
              <th className="text-[14px] leading-[21px] font-semibold text-center max-sm:min-w-[180px] md:min-w-[166px] max-md:min-w-[180px]">Complain Status</th>
              <th className="text-[14px] leading-[21px] font-semibold rounded-tr-[15px] text-center max-sm:min-w-[180px] md:min-w-[166px] max-md:min-w-[180px]">Action</th>
            </tr>
          </thead>
          <tbody>
            {complaintsData.map((complaint) => (
              <tr key={complaint.id} className="border-b border-[#F4F4F4]">
                ...
                <td className="text-center max-sm:min-w-[180px] md:min-w-[120px] max-md:min-w-[180px]">{complaint.date}</td>
                <td className="text-center max-sm:min-w-[180px] md:min-w-[120px] max-md:min-w-[180px]">
                  <button
                    className={`rounded-full text-white text-[14px] py-[5px] px-[21px] w-[100px] text-center ${getPriorityBackgroundColor(complaint.priority)}`}
                ...
                <td className="py-3 text-center max-sm:min-w-[180px] md:min-w-[120px] max-md:min-w-[180px]">
                  <button className={`text-[14px] font-medium leading-[21px] inline-block py-1 px-2 rounded-full w-[113px] h-[31px] ${getStatusStyle(complaint.status)}`}>
                    {complaint.status}
                  </button>
                </td>
      ...
      <EditModal isOpen={isEditModalOpen} onClose={onCloseEditModal} complaint={currentComplaint} onSave={onSave} />
      <ViewComplaintModal isOpen={isViewModalOpen} onClose={onCloseViewModal} complaint={currentComplaint} complainimage={complainimages} />
      <DeleteConfirmationModal isOpen={isDeleteModalOpen} onClose={onCloseDeleteModal} onDelete={onDelete} />
    </div>
  );
};
