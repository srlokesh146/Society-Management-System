      ...
      <div className="bg-white rounded-lg w-full max-w-md p-6">
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-3 mb-6">
            <img 
              src={Request.avatar || `https://ui-avatars.com/api/?name=${Request.requesterName}`} 
              alt={Request.requesterName} 
              className="w-12 h-12 rounded-full"
            />
            <div>
              <h3 className="text-lg font-semibold text-gray-900">{Request.requesterName}</h3>
              <p className="text-sm text-gray-500">{Request.requestDate || 'Aug 5, 2024'}</p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div>
              <label className="text-sm font-medium text-gray-500">Request Name</label>
              <p className="mt-1 text-base font-medium text-gray-900">{Request.requestName}</p>
            </div>

            <div>
              <label className="text-sm font-medium text-gray-500">Description</label>
              <p className="mt-1 text-base text-gray-900">{Request.description}</p>
            </div>

            <div className="flex gap-12">
              <div>
                <label className="text-sm font-medium text-gray-500">Wing</label>
                <p className="mt-1 text-base font-medium text-[#5678E9]">{Request.wing || 'A'}</p>
              </div>
              <div>
                <label className="text-sm font-medium text-gray-500">Unit</label>
                <p className="mt-1 text-base font-medium text-gray-900">{Request.unit || '1002'}</p>
              </div>
              <div>
