        ...
        <div className="relative">
          ...
          {isOpen && (
            <div className="absolute z-10 left-[-39px] bg-white border border-gray-300 rounded-lg shadow-lg mt-1 w-[149px]">
              {['Select Month', 'Last week', 'month', 'Last year'].map((option, index) => (
                <div
                  key={option}
                  onClick={option === 'Select Month' ? null : () => handleOptionClick(option)}
                  className={`flex items-center px-2 py-[3px] cursor-pointer hover:bg-gray-100 ${option === 'Select Month' ? 'text-gray-400 cursor-default' : ''}`}
                >
                  <input
                    type="radio"
                    name="period"
